...
int haEntradasControlo(Circuito* circuito, int node_index)
{
  int i;
  int controlo;
  Node* node;
  
  controlo = valorControlo( circuito, node_index );
  node = circuito->vectorNodes + node_index ;
  
  for( i = 0 ; i < node->fanin ; i++ )
    {
      if( controlo == circuito->vectorNodes[ node->inputs[ i ] ].valor )
	{
	  return 1;
	}
    }
...
int valorControlo(Circuito* circuito, int node_index)
{
  Node* node;
